...
export default class RepositoryManager {
  ...
  async init({ extraRepos, tempRepos }: { extraRepos: string[]; tempRepos?: string[] }): Promise<Repository[]> {
    log.debug("");
    log.debug(
      `=== RepositoryManager.init - extraRepos: ${JSON.stringify(extraRepos)} tempRepos: ${JSON.stringify(tempRepos)}`,
    );
  ...
  async currentDirPackage(): Promise<Package | undefined> {
    ...
    if (pkgs && pkgs.length > 1) {
      log.warn("");
      log.warn(
        t("lib.repository.repository_manager.currentMultiple", {
          chosen: pkgs[0].filePath,
          choices: pkgs.map((p) => p.filePath),
        }),
      );
      return this.repositories.currentDir.resolvePackage(pkgs[0].name);
    }

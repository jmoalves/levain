...
Deno.test("StringUtils - humanizeMillis", () => {
  assertEquals(
    StringUtils.humanizeMillis(MillisBuilder.newMillis().hours(50).minutes(35).seconds(28).millis(75).build()),
    "50h 35min 28.075s",
  );

  assertEquals(
    StringUtils.humanizeMillis(MillisBuilder.newMillis().hours(2).minutes(35).seconds(28).millis(75).build()),
    "2h 35min 28.075s",
  );

  assertEquals(
    StringUtils.humanizeMillis(MillisBuilder.newMillis().hours(2).minutes(35).seconds(28).build()),
    "2h 35min 28s",
  );

  assertEquals(
    StringUtils.humanizeMillis(MillisBuilder.newMillis().hours(2).minutes(35).millis(75).build()),
    "2h 35min 0.075s",
  );

  assertEquals(StringUtils.humanizeMillis(MillisBuilder.newMillis().hours(2).minutes(35).build()), "2h 35min");

  assertEquals(StringUtils.humanizeMillis(MillisBuilder.newMillis().hours(2).build()), "2h");

  assertEquals(StringUtils.humanizeMillis(MillisBuilder.newMillis().minutes(35).build()), "35min");

  assertEquals(StringUtils.humanizeMillis(MillisBuilder.newMillis().seconds(28).millis(75).build()), "28.075s");

  assertEquals(StringUtils.humanizeMillis(MillisBuilder.newMillis().seconds(28).build()), "28s");

  assertEquals(StringUtils.humanizeMillis(MillisBuilder.newMillis().millis(75).build()), "0.075s");

  assertEquals(StringUtils.humanizeMillis(MillisBuilder.newMillis().build()), "0.000s");

  assertEquals(StringUtils.humanizeMillis(0.029), "0.000s");
});
...
class MillisBuilder {
  ...
  public build(): number {
    return this._hours * MillisBuilder.HOURS + this._minutes * MillisBuilder.MINUTES +
      this._seconds * MillisBuilder.SECONDS + this._millis;
  }
}

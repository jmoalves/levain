...
export default class ContextMenu implements Action {
    ...
    private async templateRegistry(pkg: Package, args: any) {
        ...
        let action = `template --replace=/@@shellID@@/g --with="${args.id}" --replace=/@@shellName@@/g --with="${args.name}" --replace=/@@shell@@/g --with="${args.cmd}" --doubleBackslash \${pkg.levain.baseDir}/recipes/levain-shell.reg ${tempFilename}`;

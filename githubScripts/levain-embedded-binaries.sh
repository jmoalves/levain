...
# 7Zip - Linux - latest
sevenToolUrl=$( bash ${scriptPath}/github-get-release.sh -o ip7z -r 7zip | jq -r '.assets[].browser_download_url' | grep 'linux-x64.tar.xz' )
echo = 7Zip - Tool - ${sevenToolUrl}
zipDir=$( mktemp -d )
rm -rf $zipDir
mkdir -p $zipDir
curl -ks -o ${zipDir}/7z.tar.xz -L ${sevenToolUrl}
